...
class Calculations(private val ctx: Context) {
    ...
    fun operatorClicked(operator: String): Boolean {
        ...
        try {
            if (operator == "±") {
                numbers[numbers.size - 1] = formatNumber((numbers[numbers.size - 1].toDouble() * -1).toString())
                calc(numbers)
                return true
            }
        } catch (e: Exception) {
        ...
        if (constants.contains(operator)) {
            ...
            when (currentStatus) {
                ...
                "unary" -> currentStatus = if (numbers[numbers.size - 1] == "!" || numbers[numbers.size - 1] == "%" || numbers[numbers.size - 1] == "sq" || numbers[numbers.size - 1] == "cb") {
                    if (operator == "!" || operator == "%" || operator == "sq" || operator == "cb") {
                        var bracketOffset = 0
                        var i = numbers.size - 1
                        while (i == 0) {
    ...
    private fun getStatus(index: Int): String {
        return if (constants.contains(numbers[index])) {
            "const"
    ...
    private fun formatNumber(number: String): String {
        ...
        return if (number == null || number == "" || number.isEmpty()) {
            ...
            if (number.toDouble() == number.split("\\.").toTypedArray()[0].toLong().toDouble()) {
    ...
    private fun factorial(num: String): String {
        return try {
            ...
            if (intNum.toDouble() == doubleNum) {
                fact(intNum).toString()
            } else {
                gammaFunction(doubleNum + 1).toString()
            }
        } catch (nfe: NumberFormatException) {
            gammaFunction(java.lang.Double.valueOf(num) + 1).toString()
        }
    }
    ...
    private fun percent(num: String): String {
        return (num.toDouble() / 100).toString()
    }

    private fun ln(num: String): String {
        return Math.log(num.toDouble()).toString()
    }

    private fun log(num: String): String {
        return Math.log10(num.toDouble()).toString()
    }
    ...
    private fun deg(num: String): String {
        ...
        val dms = num.split("\\.").toTypedArray()
        ...
        val ms = m.toDouble() / 60 + s.toDouble() / 3600
    ...
    private fun sin(value: String): String {
        return Math.sin(value.toDouble()).toString()
    }

    private fun cos(value: String): String {
        return Math.cos(value.toDouble()).toString()
    }

    private fun tan(value: String): String {
        return Math.tan(value.toDouble()).toString()
    }

    private fun asin(value: String): String {
        return Math.asin(value.toDouble()).toString()
    }

    private fun acos(value: String): String {
        return Math.acos(value.toDouble()).toString()
    }

    private fun atan(value: String): String {
        return Math.atan(value.toDouble()).toString()
    }

    private fun sinh(value: String): String {
        return Math.sinh(value.toDouble()).toString()
    }

    private fun cosh(value: String): String {
        return Math.cosh(value.toDouble()).toString()
    }

    private fun tanh(value: String): String {
        return Math.tanh(value.toDouble()).toString()
    }
